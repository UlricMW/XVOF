...
class NodeTest(unittest.TestCase):
    ...
    def test_init(self):
        ...
        msg = "Par dï¿½faut, les vecteurs vitesses doivent être égaux au vecteur nul!"
        ...
        np.testing.assert_array_equal(default_node_2d.masse, np.array([[0., 0.], ]),
                                      "La masse doit être initalisée à 0!")

        # Si la dimension du noeud ne correspond pas à celle des vecteurs positions
        # et vitesses alors une exception de type SystemExit (pas top) doit être levée
        ...
        # Les vecteurs position et vitesse initiaux doivent être ceux passés en argument
    ...
    def test_compute_new_coordinates(self):
        """
        Test de la méthode Node.compute_new_coordinates()
        """
        self.my_node.compute_new_coordinates(np.array([True]), delta_t=0.5e-06)
